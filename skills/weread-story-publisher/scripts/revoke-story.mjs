...
if (url.protocol !== 'https:' || url.username || url.password || !slug || !/^[A-Za-z0-9_-]{43}$/.test(credential)) throw new Error('撤销文件格式无效。');

const response = await fetch(`${url.origin}/api/stories/${slug}`, { method: 'DELETE', signal: AbortSignal.timeout(30000), headers: { 'X-Story-Revoke': credential } });
