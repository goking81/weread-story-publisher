...
At the beginning of a normal Skill run, quietly run `node <skill-directory>/scripts/update-skill.mjs --background`. It checks at most once per day and automatically installs only a newer public GitHub version, including its locked runtime dependency. A network or update failure must not interrupt the user's Story workflow. Do not display the check unless the user asks about versioning or an update actually changes behavior. Use `--check` to report the available version, or `--apply` to force an immediate update check.
