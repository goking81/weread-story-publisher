...
![微信读书 API Key 的安全配置流程示意](docs/api-key-guide.svg)

`WEREAD_API_KEY` 是微信读书 **Agent API** 的访问凭据，通常以 `wrk-` 开头。它不是微信登录密码、短信验证码、Cookie 或抓包数据。

本仓库只使用 Key，**不能创建、领取、重置或代替微信读书签发 Key**。Key 的获取入口取决于用户当前使用的微信读书 Agent API 授权渠道，通常是：

1. 用户已获准使用的微信读书 Agent API 控制台中的 API Key 页面；或
2. 为用户提供 Agent API 能力的组织、产品或管理员签发的个人 Key。

请只在该授权渠道中创建或复制自己的 `wrk-…` Key。如果找不到“API Key / 密钥 / Agent API”入口，或管理员没有为您开通此能力，说明当前账号还不能使用本 Skill；不要通过微信密码、短信验证码、浏览器 Cookie、抓包或第三方“取 Key”工具绕过授权。
